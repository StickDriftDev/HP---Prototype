...
func _physics_process(delta: float) -> void:
	...
	if not is_charging_jump and (is_on_floor() or is_wall_running):
		...
		apply_floor_snap()
	move_and_slide() # Trocado para chamada direta
...
func _handle_landing(_delta: float) -> void:
	if is_wall_running or not is_on_floor() or get_floor_normal().dot(Vector3.UP) < 0.5:
		return
...
func _update_visuals_and_juice(delta: float) -> void:
	...
	if board_target:
		...
		if is_drifting:
			target_tilt *= drift_lean_multiplier
